...
//DIMAS_____________________________________________________________

	public function calendar_cal() {
		if ($this->session->userdata('admin_valid') == FALSE && $this->session->userdata('admin_id') == "") {
			redirect("admin/login");
		...
		$a['page']	= "view_calendars";
		
		$this->load->view('admin/aaa', $a);
	}


	public function form_proposal() {
		if ($this->session->userdata('admin_valid') == FALSE && $this->session->userdata('admin_id') == "") {
			redirect("admin/login");
...
//DIMAS_____________________________________________________________
	...
	public function klas_surat() {
		...
		/* pagination */	
		$total_row		= $this->db->query("SELECT * FROM t_acara")->num_rows();
		$per_page		= 10;
		...
		$id_acara				= addslashes($this->input->post('id_acara'));
		$nama_acara				= addslashes($this->input->post('nama_acara'));
		$Jenis_acara			= addslashes($this->input->post('Jenis_acara'));
		$tanggal_acara			= addslashes($this->input->post('tanggal_acara'));
	
		$cari					= addslashes($this->input->post('q'));

		
		if ($mau_ke == "cari") {
			$a['data']		= $this->db->query("SELECT * FROM t_acara WHERE nama_acara LIKE '%$cari%' OR WHERE tanggal_acara LIKE '%$cari%' OR Jenis_acara LIKE '%$cari%' ORDER BY id DESC")->result();
			$a['page']		= "l_klas_surat";
		} else if ($mau_ke == "add") {
			$a['page']		= "f_klas_surat";
		} else if ($mau_ke == "edt") {
			$a['datpil']	= $this->db->query("SELECT * FROM t_acara WHERE id_acara = '$idu'")->row();	
			$a['page']		= "f_klas_surat";
		} else if ($mau_ke == "act_edt") {
			$this->db->query("UPDATE t_acara SET nama_acara = '$nama_acara', Jenis_acara = '$Jenis_acara' WHERE id_acara = '$id_acara'");
			$this->session->set_flashdata("k", "<div class=\"alert alert-success\" id_acara=\"alert\">Data has been updated</div>");			
			redirect('admin/klas_surat');
		} else {
			$a['data']		= $this->db->query("SELECT * FROM t_acara LIMIT $awal, $akhir ")->result();
			$a['page']		= "l_klas_surat";
		}
	...
	public function surat_masuk() {
		...
		$a['list_kode_surat'] = $this->web_model->get_dropdown_kode_surat();
		/* pagination */	
		...
		$kode					= addslashes($this->input->post('kode_surat'));
		...
		if ($mau_ke == "del") {
			...
			$a['data']		= $this->db->query("SELECT * FROM t_surat_masuk ORDER BY no_agenda DESC LIMIT $awal, $akhir ")->result();
			$a['page']		= "l_surat_masuk";
		}
	...
	public function surat_keluar() {
		...
		$no_surat3				= addslashes($this->input->post('no_surat3'));
		$no_surat4				= addslashes($this->input->post('no_surat4'));
		...
		if ($mau_ke == "del") {
			...
			if ($this->upload->do_upload('file_surat')) {
				...
				$this->db->query("INSERT INTO t_surat_keluar VALUES (NULL, '$kode', '$no_agenda', '$uraian', '$dari', '$no_surat', '$tgl_surat', NOW(), '$ket', '".$up_data['file_name']."', '".$this->session->userdata('admin_id')."', '$no_surat1', '$no_surat2', '$no_surat3', '$no_surat4')");
			} else {
				$this->db->query("INSERT INTO t_surat_keluar VALUES (NULL, '$kode', '$no_agenda', '$uraian', '$dari', '$no_surat', '$tgl_surat', NOW(), '$ket', '',  '".$this->session->userdata('admin_id')."', '$no_surat1', '$no_surat2', '$no_surat3', '$no_surat4')");
			}		
			...
			if ($this->upload->do_upload('file_surat')) {
				...
				$this->db->query("UPDATE t_surat_keluar SET no_agenda = '$no_agenda', kode = '$kode', isi_ringkas = '$uraian', tujuan = '$dari', no_surat = '$no_surat', tgl_surat = '$tgl_surat', keterangan = '$ket', file = '".$up_data['file_name']."' , no_surat1 = '$no_surat1' , no_surat2 = '$no_surat2' , no_surat3 = '$no_surat3' , no_surat4 = '$no_surat4' WHERE id = '$idp'");
			} else {
				$this->db->query("UPDATE t_surat_keluar SET no_agenda = '$no_agenda', kode = '$kode', isi_ringkas = '$uraian', tujuan = '$dari', no_surat = '$no_surat', tgl_surat = '$tgl_surat', keterangan = '$ket' , no_surat1 = '$no_surat1' , no_surat2 = '$no_surat2' , no_surat3 = '$no_surat3' , no_surat4 = '$no_surat4' WHERE id = '$idp'");
			}	
			
			$this->session->set_flashdata("k", "<div class=\"alert alert-success\" id=\"alert\">Data has been updated ".$this->upload->display_errors()."</div>");			
			redirect('admin/surat_keluar');
		} else {
			$a['data']		= $this->db->query("SELECT * FROM t_surat_keluar ORDER BY no_agenda DESC LIMIT $awal, $akhir")->result();
			$a['page']		= "l_surat_keluar";
		}
	...
	public function pengguna() {
		...
		$ketua_instansi			= addslashes($this->input->post('ketua_instansi'));
		...
		if ($mau_ke == "act_edt") {
			if ($this->upload->do_upload('logo')) {
				...
				$this->db->query("UPDATE tr_instansi SET nama = '$nama', alamat = '$alamat', ketua_instansi = '$ketua_instansi', nidn = '$nidn', logo = '".$up_data['file_name']."' WHERE id = '$idp'");

			} else {
				$this->db->query("UPDATE tr_instansi SET nama = '$nama', alamat = '$alamat', ketua_instansi = '$ketua_instansi', nidn = '$nidn' WHERE id = '$idp'");
			}		
	...
	public function get_klasifikasi() {
		...
		$data 				=  $this->db->query("SELECT id_acara, nama_acara FROM ref_klasifikasi WHERE kode LIKE '%$kode%' ORDER BY id ASC")->result();
	...
	public function do_login() {
        ...
        if($j_cek == 1) {
           ...
           $this->session->set_userdata($data);
            if ($this->session->userdata('admin_status') == "0"){
            	$a = [
            		'page' => 'l_profil_organisasi'];
            	$this->load->view('admin/aaa', $a);
            	} else {
            	redirect('admin');
            }
